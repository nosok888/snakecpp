void logic(Map* _map, Apple* _apple, Snake* _snake){
	...
	if(_snake->getPositionOfSnakePart(0)[0] == max_x && _snake->getDirection() == Direction::__right__){
		int _y = _snake->getPositionOfSnakePart(0)[1];
		_snake->setNewHeadPositionOfSnake(0, _y);
		
	}else if(_snake->getPositionOfSnakePart(0)[0] == min_x-1 && _snake->getDirection() == Direction::__left__){
		int _y = _snake->getPositionOfSnakePart(0)[1];
		_snake->setNewHeadPositionOfSnake(max_x-1, _y);
		
	}else if(_snake->getPositionOfSnakePart(0)[1] == max_y && _snake->getDirection() == Direction::__down__){
		int _x = _snake->getPositionOfSnakePart(0)[0];
		_snake->setNewHeadPositionOfSnake(_x, 0);
		
	}else if(_snake->getPositionOfSnakePart(0)[1] == min_y-1 && _snake->getDirection() == Direction::__up__){
		int _x = _snake->getPositionOfSnakePart(0)[0];
